...
def error(mesg):
    #print >>sys.stderr, ">>>>> (Error): {0}".format(mesg)
    print(">>>>> (Error): {0}".format(mesg), file=sys.stderr)
    sys.exit(1)

def warn(mesg):
    #print >>sys.stderr, ">> (Warning): {0}".format(mesg)
...
def get_agent_sql(data):
    ...
    if name not in agents.keys():
        error("Unknown agent found: {0}".format(name))
    ...
    for v in vars.keys():
        ...
        if v not in agents[name]:
            error("Unknown agent memory : {0}.{1}".format(name, v))
        keys.append(v)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog='gendb.py', description='Script creates SQLite db file from corresponding XML files in many-to-one fashion, and skips missing Agents.')
